...
func main() {
	...
	for i1, v1 := range songs {
		...
		q := v1.Snippet.Title
		if generated {
			q += " " + strings.TrimSuffix(v1.Snippet.VideoOwnerChannelTitle, " - Topic")
		}

		// Search for the song in spotify with the query.
		searchResults, err := spotifyClient.Search(q, spotify.SearchTypeTrack)
		if err != nil {
		...
		for _, v := range searchResults.Tracks.Tracks {
			...
			if strings.TrimSuffix(v1.Snippet.VideoOwnerChannelTitle, " - Topic") == v.Artists[0].Name {
	...
	// Done!
	fmt.Println("\nDone!")
	fmt.Println("You mignt need to make some tweeks your self.")
	fmt.Printf("How did I do? https://open.spotify.com/playlist/%s\n", spotifyPlaylistID)
}
